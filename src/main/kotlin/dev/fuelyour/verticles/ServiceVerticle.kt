...
class ServiceVerticle : AbstractVerticle(), KoinComponent {
  ...
  override fun rxStart(): Completable =
    configureConnections(vertx).doOnSuccess { pgPool ->
      ...
      vertx.eventBus().consumer<JsonObject>("db.$dbName") { message ->
        ...
        if (resourceObj != null) {
          val method = resourceObj
            .javaClass
            .methods
            .filter { it.name.toLowerCase() == action.toLowerCase() }
            .first()
          (method.invoke(
            resourceObj,
            *params(body, method.parameters)
          ) as Single<*>).subscribe(
            { message.reply(it) },
            { message.reply("error") })
        } else {
          throw IllegalArgumentException(
            "invalid action: $action, on resource: $resource"
          )
        }
      }
    }.ignoreElement()
  ...
  private fun modelResources(pool: PgPool) =
    Reflections(this.javaClass.`package`.name.substringBeforeLast('.'))
      .getSubTypesOf(Repository::class.java).map {
        val key = it
          .simpleName
          .toLowerCase()
          .removeSuffix("repo")
  ...
  private fun configureConnections(vertx: Vertx): Single<PgPool> =
